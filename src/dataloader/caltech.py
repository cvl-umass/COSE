# Code below is based on https://pytorch.org/vision/stable/_modules/torchvision/datasets/caltech.html
...
class Caltech101(VisionDataset):
    ...
    def __init__(
        ...
        if self._split == "train":
            self.index, _, self.targets, _ = train_test_split(self.index, self.targets, train_size=0.8, random_state=0)
        else:
            _, self.index, _, self.targets = train_test_split(self.index, self.targets, train_size=0.8, random_state=0)

        self.len_orig = len(self.targets)  # number of orig images

        if (self.rot_vals_deg is not None) or to_bgr or to_rrr or to_double_data_only:
            self.index.extend(self.index)
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if img.mode == "L": # Convert grayscale to RBG
            img = img.convert("RGB")
